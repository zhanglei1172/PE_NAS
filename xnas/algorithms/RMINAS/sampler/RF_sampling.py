        ...
        if self.space == 'nasbench201':
            ...
            def get_newX(r, n, i):
                ...
                for j in range(n):
                    ...
                    r[i] = j
                    get_newX(r, n, i+1)
            get_newX([0,0,0,0,0,0], 5, 0)
                    # r = [tmp]
            idx = (rf.predict_proba(res)[:,-1]).argmax()
            optim = res[idx]
            result = []
            if method == 'sum':
...
#             print(sum_max)
            ...
            assert method == 'sum', 'only sum is supported in mb.'
            with open('./nbm_rmi_res.pkl','wb') as f:
                pickle.dump(self.trained_arch,f)
            features = []
            labels = []
            for i in self.trained_arch:
                features.append(i['arch'].ravel())
