...
class SPOS_nb201_CNN(NASBench201CNN):
    ...
    def forward(self, inputs, weight):
        feature = self.stem(inputs)
        _it = 0
        if len(weight) < self.spos_all_edge_num:
            weight = np.tile(weight, self.spos_all_edge_num//len(weight))
        for i, cell in enumerate(self.cells):
            if isinstance(cell, ResNetBasicblock):
                feature = cell(feature)
            else:
...
def _NASBench201():
    ...
    return NASBench201CNN(
        ...
        # affine=cfg.SPACE.affine,
        # track_running_stats=cfg.SPACE.track_running_stats
    )
