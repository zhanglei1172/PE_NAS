...
from xnas.spaces.NASBench1Shot1.layers import ConvBnRelu, Conv3x3BnRelu, Conv1x1BnRelu, StdConv, FactorizedReduce
...
class Infer_MixedOp(nn.Module):

    def __init__(self, C, stride, weights):
        super(Infer_MixedOp, self).__init__()
        # self._ops = nn.ModuleList()
        # for primitive in PRIMITIVES:
        #     op = OPS[primitive](C, stride, False)
        #     self._ops.append(op)
        ...
        self.op = OPS[primitive](C, stride, False)
    def forward(self, x):
        return self.op(x)
...
class Infer_ChoiceBlock(nn.Module):

    def __init__(self, C_in,input_weights, weights):
        super(Infer_ChoiceBlock, self).__init__()
        self.mixed_op = Infer_MixedOp(C_in, stride=1, weights=weights)

        self.input_weights = None if input_weights is None else input_weights.squeeze(0).astype(bool)

    def forward(self, inputs):
        if self.input_weights is not None:
            inputs_ = []
            for i in range(len(self.input_weights)):
                if self.input_weights[i]:
                    inputs_.append(inputs[i])
        else:
            inputs_ = inputs

        input_to_mixed_op = sum(inputs_)
